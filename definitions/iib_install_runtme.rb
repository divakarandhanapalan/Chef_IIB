...
define :iib_install_runtime  do
  ...
  group "Create IIB admin group mqbrkrs" do
	...
	log "IBM Integration Bus runtime install log can be found in the /opt/ibm/mqsi/10.0.0.10 directory" do 
		level :info
	end
  ...
  execute "Run IBM Integration Bus installer" do
    ...
    returns [0,127]
    cwd "#{unpack_dir}"
    command "sudo ./iib make registry global accept license silently"
  end
  #
  ...
  execute "Update users profile to include curent IIB environment (mqsiprofile)" do
    user "root"
    returns [0]
    command "sed -i '$a source /var/chef/cache/iib_installer/server/bin/mqsiprofile' #{home}/.bash_profile"
  end
  ...
  execute "Update users .bashrc to include curent IIB environment (mqsiprofile)" do
    user "root"
    returns [0]
    command "sed -i '$a source /var/chef/cache/iib_installer/server/bin/mqsiprofile' #{home}/.bashrc"
end
end
